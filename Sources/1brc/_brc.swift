...
struct _brc {
    static func main() throws {
        ...
        while let chunk = try handle.read(upToCount: bufferSize) {
            let lastNewLineIndex = findLastNewLine(in: chunk)
    ...
    static func processNewLineAlignedChunk(_ chunk: Data, results: inout Results) {
        ...
        // TODO: investigate span
        let span = chunk.span.indices
        var bytePosition = span.startIndex
        while true {
            guard let semiIndex = chunk[bytePosition...].firstIndex(of: .semicolon) else { break }
            let cityBytes = chunk[bytePosition..<semiIndex]
            bytePosition = semiIndex.advanced(by: 1)
            
            guard let newLineIndex = chunk[bytePosition...].firstIndex(of: .newLineByte) else {
                fatalError("Expected to always find a new line byte here: \(String(data: chunk[bytePosition...], encoding: .utf8) ?? "????")")
            }
            let temperatureBytes = chunk[bytePosition..<newLineIndex]
            bytePosition = newLineIndex.advanced(by: 1)
    ...
    static func findLastNewLine(in data: Data) -> Int {
